...
function FearGreedGaugeLarge({ value, classification }: { value: number; classification: string }) {
  const getLabelColor = (val: number, isHex = false) => {
    if (val <= 25) return isHex ? '#EF4444' : '';
    if (val <= 45) return isHex ? '#F97316' : '';
    if (val <= 55) return isHex ? '#EAB308' : 'text-yellow-500';
    if (val <= 75) return isHex ? '#10B981' : '';
    return isHex ? '#22C55E' : '';
  };
      ...
      <div className="relative flex items-center justify-center">
        <svg viewBox="20 20 140 100" className="w-full max-w-[280px] md:max-w-[360px] overflow-visible drop-shadow-2xl">
          ...
          <text
            ...
            className="font-inter tracking-tighter drop-shadow-sm"
          >
            {value}
          </text>
        </svg>
...
<h3 className="title-newtab">{title}</h3>
...
<h2 className="title-newtab text-3xl md:text-4xl mb-4">
        ...
        {/* Por que Importa */}
        <section>
<h2 className="title-newtab text-2xl md:text-3xl mb-6">
...
<h2 className="title-newtab text-xl md:text-2xl mb-4">
              Limitações e Riscos
            </h2>
            <div className="grid md:grid-cols-2 gap-6">
        ...
        {/* Fontes */}
        <section>
<h2 className="title-newtab text-lg text-[var(--text-tertiary)] mb-4">Fontes e Referências</h2>
