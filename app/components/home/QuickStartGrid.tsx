...
import { faChartLine, faGraduationCap, faNewspaper, faArrowRight, faCoins, faTools, faLightbulb } from '@fortawesome/free-solid-svg-icons';
...
const initialCards = [
  ...
  },
  {
    ...
    icon: faLightbulb,
    title: 'Curiosidades',
    description: 'Fatos e marcos cripto',
    theme: 'default'
  }
];
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-start text-center gap-4 pt-4">
          ...
          <h3 className={`title-newtab text-base transition-colors ${isActive ? 'text-[var(--brand-primary)]' : ' group-hover:text-[var(--brand-primary)]'}`}>
            {card.title}
          </h3>
        </div>
      </ZenithCard>
      ...
      <section className="py-8">
        ...
        <div className="flex flex-wrap justify-start md:justify-center gap-4 lg:gap-6">
          {initialCards.map((card) => {
            ...
            return (
              <div key={card.id} className="w-[calc(50%-0.5rem)] lg:w-[calc(16.66%-1.25rem)] md:w-[calc(33.33%-1rem)] flex-grow-0 flex-shrink-0">
                <SortableCard
                  card={card}
                  id={card.id}
