        ...
        <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-5xl mx-auto">
                <div className="text-center mb-16">
                    <h1 className="text-4xl md:text-6xl font-bold mb-6 font-[family-name:var(--font-poppins)] text-[var(--text-primary)]">
                        Roadmap Estratégico
                    </h1>
                                        ...
                                        <div className={`glass-card p-8 rounded-3xl border-t-4 ${phase.borderColor} transition-all duration-500 hover:shadow-2xl hover:scale-[1.02] relative overflow-hidden`}>
                                            ...
                                            <div className="flex items-center justify-between mb-6 relative z-10">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {phase.status === 'completed' && <span className="text-green-500 text-sm font-bold flex items-center gap-1">Concluído</span>}
                                                    {phase.status === 'active' && <span className="text-teal-500 text-sm font-bold flex items-center gap-1 animate-pulse">Em Andamento</span>}
                                                    {phase.status === 'upcoming' && <span className="text-[var(--text-tertiary)] text-sm font-bold flex items-center gap-1">Futuro</span>}
                                                </div>
                                            </div>

                                            <h3 className="text-2xl font-bold mb-4 text-[var(--text-primary)] relative z-10">{phase.title}</h3>
