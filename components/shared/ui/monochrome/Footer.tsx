...
export default function Footer() {
  ...
  return (
    <footer className="glass border-t border-[var(--border-light)] mt-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr] gap-8 mb-12">
          ...
          <div className="flex flex-col gap-4">
            <Link href="/" className="flex items-center gap-3 w-fit group">
              ...
              />
              <span className="text-xl font-bold text-[var(--text-primary)] group-hover:opacity-80 transition-opacity">
                $MILAGRE
              </span>
            </Link>
          ...
          <div className="flex flex-col">
            <h3 className="text-sm font-bold text-[var(--text-primary)] uppercase tracking-wider mb-4">Produto</h3>
            <nav className="flex flex-col gap-3" aria-label="Links do produto">
              {footerLinks.produto.map((link) => (
                <Link key={link.href} href={link.href} className="text-sm text-[var(--text-secondary)] hover:text-[var(--brand-primary)] transition-colors w-fit">
                  {link.label}
                </Link>
              ))}
            </nav>
          ...
          {/* Community Column */}
          <div className="flex flex-col">
            <h3 className="text-sm font-bold text-[var(--text-primary)] uppercase tracking-wider mb-4">Comunidade</h3>
            <nav className="flex flex-col gap-3" aria-label="Links da comunidade">
              {footerLinks.comunidade.map((link) => (
                link.external ? (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sm text-[var(--text-secondary)] hover:text-[var(--brand-primary)] transition-colors w-fit"
                  >
                    {link.label}
                  </a>
                ) : (
          ...
          {/* Legal Column */}
          <div className="flex flex-col">
            <h3 className="text-sm font-bold text-[var(--text-primary)] uppercase tracking-wider mb-4">Legal</h3>
