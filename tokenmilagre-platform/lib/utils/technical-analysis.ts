...
export const calculateTrendSignal = (
    ...
    if (score <= -3) label = 'Sobrecomprado - Risco de Correção';
    else if (score < 0) label = 'Preço Elevado - Cautela';
    else if (score === 0) label = 'Neutro - Aguardar Definição';
    else if (score < 3) label = 'Preço Atrativo - Considerar Entrada';
    else label = 'Sobrevendido - Forte Oportunidade';

    // Calculate Exact Score for Gauge (Smoother)
    // RSI: Continuous from (50 - RSI) / 10
    // e.g. RSI 30 -> +2.0, RSI 70 -> -2.0
    const rsiExact = (50 - rsi) / 10;
