...
export default function HomePage() {
  ...
  const renderSection = (id: string) => {
    switch (id) {
      case 'ticker':
        return <ZenithMarketTicker marketData={marketData} />;
      case 'hud':
        return (
          <ZenithHeroHUD
  ...
  return (
    <div className="min-h-screen relative">
      <div className="container mx-auto px-6 md:px-10 py-8 relative z-10">
        ...
        <div className="mt-12 flex justify-center pb-8 border-t border-[var(--border-light)] pt-8">
          <button
            onClick={() => {
              if (window.confirm('Deseja restaurar a ordem original das sessões?')) {
                localStorage.removeItem(STORAGE_KEY);
                localStorage.removeItem('zenith_ticker_order'); // Also reset ticker if needed, or keep separate
                localStorage.removeItem('zenith_sidebar_order'); // Optional: reset all
                window.location.reload();
              }
            }}
            className="text-xs font-bold text-[var(--text-tertiary)] hover:text-[var(--brand-primary)] transition-colors uppercase tracking-widest opacity-50 hover:opacity-100"
          >
